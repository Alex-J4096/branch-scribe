...
### Step 100: 增加 Block 工具窗口缩放与字号调节

- Block 工具浮动窗口新增右下角拖拽缩放，并限制最小宽高，避免正文编辑器和 LLM Chatbox 在过小尺寸下错乱。
- 项目主页面菜单新增“显示设置”二级菜单，集中提供 13–22px 内容字号调节、浅色/深色/跟随系统主题和衬线字体开关；Block 工具标题栏不再放置全局入口。
- 显示偏好提升为应用级设置：主题覆盖工作台、独立 Block 页面和管理页面，字号与字体覆盖所有正文、对话、摘要及上下文阅读内容。
- 补充深色模式的完整视觉配合，覆盖项目列表、工作台侧栏与画布、图节点、弹窗、管理页表单与列表、选中状态、标签和危险操作。
- Block 工具进入工作台时默认收起且不显示启动器；用户选中 Block 后自动展开，并将首次位置限制在画布安全区域内，避免顶部被工作台工具栏遮挡。
- 全局“显示设置”入口同时放置在项目列表和工作台顶部主菜单，进入项目后仍可直接切换显示偏好。
- 深色模式补齐历史版本列表、Diff Viewer、LLM 消息流、消息编辑、Chatbox、模型选择、二级工具菜单和上下文面板，消除 Block 工具中的浅色残留。
- 修复运行时切换主题后 Block 工具标题栏和标签栏仍保留旧主题、刷新后才正常的问题；移除组件内重复主题状态，所有表面统一直接响应全局主题属性。
- 窗口尺寸和全局显示偏好保存到浏览器本地存储，再次进入时自动恢复，并兼容迁移旧 Block 工具设置。
- 完成后勾选 `ARCHITECTURE.md` 中对应任务。
