...
BranchScribe 是一个基于 LLM 的非线性故事创作工具。
...
* [x] 在 Block LLM 对话的 assistant 消息下展示总耗时、输入/输出 Token、首次返回延时和生成速度。
...
* [x] 删除整个 LLM 对话前进行二次确认，取消生成操作保持直接执行。
...
* [x] 重新生成历史回复时支持临时选择其他可用模型，且不改变 Chatbox 默认模型。
* [x] LLM 请求失败后恢复 User 消息的正常工具栏与删除能力，不保留不可操作的 pending 消息。
* [x] 流式生成中断且 User 尚无 Agent 回复时，支持从该 User 原位续接生成且不重复保存 User 消息。
...
* [x] Block 工具浮动窗口默认收起、选中 Block 后安全定位展开，并支持拖拽缩放和最小尺寸保护；项目列表与工作台主菜单提供全局显示设置。
...
* [x] 提示用户哪些摘要已过期。
* [x] 增加分支摘要设置菜单，支持逐 Block 选择正文、摘要或排除。
* [x] 在摘要设置中集中选择模型、摘要 Prompt，并提供 Prompt 管理入口。

### 摘要编排增强

* [x] 分支摘要 API 接收并校验逐 Block 上下文构成。
* [x] 摘要快照记录本次上下文构成与 Prompt，刷新时可复用和调整。
...
* [x] 实现 check_consistency task。
* [x] 实现角色状态记录。
* [x] 实现伏笔记录。
* [x] 实现时间线事件记录。
* [x] 支持从 block 中提取事件，可选。
* [x] 支持从 block 中提取角色状态，可选。
* [x] 支持检查当前正文是否违反 canon。

### 前端任务

* [x] 实现一致性检查按钮。
* [x] 显示冲突列表。
* [x] 支持跳转到相关 canon entity。
* [x] 支持跳转到相关 block。
* [x] 实现角色状态面板。
* [x] 实现伏笔面板。
* [x] 实现时间线视图，可选。
